...
def scraper_tous_ats(mots_cles=""):
    offres = []
    for e in [{"nom": "Mistral AI", "slug": "mistral"}, {"nom": "Qonto", "slug": "qonto"}, {"nom": "Pennylane", "slug": "pennylane"}, {"nom": "Swile", "slug": "swile"}]:
        try:
            ...
            for job in r.json():
                if mots_cles and mots_cles.lower() not in job.get("text", "").lower():
                    continue
                offres.append({"id": f"lever_{job.get('id')}", "titre": job.get("text"), "entreprise": e["nom"], "lieu": job.get("categories", {}).get("location", "Non précisé"), "contrat": job.get("categories", {}).get("commitment", "Non précisé"), "salaire": "Non précisé", "description": job.get("descriptionPlain", "")[:300], "date_publication": datetime.utcfromtimestamp(job.get("createdAt", 0) / 1000).isoformat(), "url": job.get("hostedUrl"), "source": f"Lever ({e['nom']})", "competences": []})
        except: pass
    for e in [{"nom": "Doctolib", "slug": "doctolib"}, {"nom": "Alma", "slug": "alma"}]:
        try:
            ...
            for job in r.json().get("jobs", []):
                if mots_cles and mots_cles.lower() not in job.get("title", "").lower():
                    continue
                offres.append({"id": f"greenhouse_{job.get('id')}", "titre": job.get("title"), "entreprise": e["nom"], "lieu": job.get("location", {}).get("name", "Non précisé"), "contrat": "Non précisé", "salaire": "Non précisé", "description": "", "date_publication": job.get("updated_at", ""), "url": job.get("absolute_url"), "source": f"Greenhouse ({e['nom']})", "competences": []})
        except: pass
    return offres
